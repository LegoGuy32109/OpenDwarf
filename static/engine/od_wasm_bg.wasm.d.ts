/* tslint:disable */
/* eslint-disable */
export const memory: WebAssembly.Memory;
export const __wbg_uiengine_free: (a: number, b: number) => void;
export const uiengine_new: () => number;
export const uiengine_rect_ptr: (a: number) => number;
export const uiengine_rect_capacity: (a: number) => number;
export const uiengine_glyph_ptr: (a: number) => number;
export const uiengine_glyph_capacity: (a: number) => number;
export const uiengine_world_atlas_ptr: (a: number) => number;
export const uiengine_world_atlas_capacity: (a: number) => number;
export const uiengine_world_solid_ptr: (a: number) => number;
export const uiengine_world_solid_capacity: (a: number) => number;
export const uiengine_drawlist_ptr: (a: number) => number;
export const uiengine_drawlist_capacity: (a: number) => number;
export const uiengine_input_ptr: (a: number) => number;
export const uiengine_input_capacity: (a: number) => number;
export const uiengine_view_globals_ptr: (a: number) => number;
export const uiengine_dropped_rects: (a: number) => number;
export const uiengine_dropped_glyphs: (a: number) => number;
export const uiengine_dropped_draw_cmds: (a: number) => number;
export const uiengine_abi_drawcmd_stride: (a: number) => number;
export const uiengine_abi_rect_stride_floats: (a: number) => number;
export const uiengine_abi_glyph_stride: (a: number) => number;
export const uiengine_abi_glyph_stride_floats: (a: number) => number;
export const uiengine_abi_program_rect: (a: number) => number;
export const uiengine_abi_program_text: (a: number) => number;
export const uiengine_abi_program_world_atlas_quad: (a: number) => number;
export const uiengine_abi_program_world_solid_quad: (a: number) => number;
export const uiengine_debug_snapshot_json: (a: number) => [number, number];
export const uiengine_debug_world_snapshot_json: (a: number) => [number, number];
export const uiengine_debug_draw_hash: (a: number) => [number, number];
export const uiengine_hydrate_settings: (a: number, b: number, c: number) => void;
export const uiengine_reset_for_harness: (a: number) => void;
export const uiengine_reset_play_world: (a: number) => void;
export const uiengine_step_sim_ticks: (a: number, b: number) => void;
export const uiengine_import_replay_json: (a: number, b: number, c: number) => [number, number, number, number];
export const uiengine_frame: (a: number) => number;
export const uiengine_abi_rect_stride: (a: number) => number;
export const uiengine_view_globals_capacity: (a: number) => number;
export const uiengine_abi_world_atlas_stride: (a: number) => number;
export const uiengine_abi_world_atlas_stride_floats: (a: number) => number;
export const uiengine_abi_world_solid_stride: (a: number) => number;
export const uiengine_abi_world_solid_stride_floats: (a: number) => number;
export const __wbindgen_externrefs: WebAssembly.Table;
export const __wbindgen_free: (a: number, b: number, c: number) => void;
export const __wbindgen_malloc: (a: number, b: number) => number;
export const __externref_table_dealloc: (a: number) => void;
export const __wbindgen_start: () => void;
