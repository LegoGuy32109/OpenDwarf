...
use bevy::sprite_render::{
  AlphaMode2d, TileData, TilemapChunk, TilemapChunkTileData,
};
...
fn setup(mut commands: Commands, asset_server: Res<AssetServer>) {
  ...
  let dwarf_coordinates =
    MapCoordinates::new(IVec3::ZERO, uvec3(chunk_size.x, chunk_size.y, 1));
  let dwarf_transform =
    chunk.calculate_tile_transform(dwarf_coordinates.as_uvec2());
...
fn consume_action(
  ...
  mut entities_with_transforms: Query<
    (&mut Transform, &mut MapCoordinates),
    With<Player>,
  >,
) {
  ...
  for (action_entity, mut action) in &mut actions_in_progress {
    ...
    let current_tile_transform =
      tilemap.calculate_tile_transform(current_tile_index);
    let destination_tile_transform =
      tilemap.calculate_tile_transform(destination_tile_index);
...
fn debug_menu(
  ...
  if let Ok(bundle) = maybe_debug_text {
    ...
    let pressed_output =
      format_keys("Pressed Keys", keyboard_input.get_pressed().copied());
    ...
    text.0 =
      [just_pressed_output, pressed_output, just_released_output].join("\n");
...
fn keyboard_movement(
  ...
  let pressed_movement_keys: HashSet<KeyCode> = keyboard_input
    ...
    .copied()
    .collect();
  let mut movement_keys_sorted: Vec<KeyCode> =
    pressed_movement_keys.into_iter().collect();
