...
impl InputState {
    ...
    pub(crate) fn apply_key_events<I>(&mut self, key_events: I, typing: bool)
    where
        I: IntoIterator<Item = KeyboardInput>,
    {
        let bindings = self.command_bindings.clone();
        let prev_pressed = self.pressed_keys.clone();
        ...
        for event in key_events.into_iter() {
            ...
            for binding in &bindings {
                if binding.matches(&event, &next_pressed, typing) {
        ...
        self.commands_triggered = commands_triggered;
        self.just_pressed_keys = next_just_pressed;
        self.pressed_keys = next_pressed;
        self.key_events = filtered_events;
    }
}
