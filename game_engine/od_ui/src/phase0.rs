...
pub fn build_phase0_demo_frame(
    ...
    if !push_draw_cmd(draw_cmds, &mut draw_count, ProgramId::Rect, 0, rect_count) {
        dropped_draw_cmds += 1;
    }
    ...
    if !push_draw_cmd(draw_cmds, &mut draw_count, ProgramId::Text, 0, glyph_count) {
        dropped_draw_cmds += 1;
    }
...
mod tests {
    ...
    fn demo_frame_emits_bordered_rect_and_hello() {
        ...
        let counts =
            build_phase0_demo_frame(800, 600, 1.0, &mut rects, &mut glyphs, &mut draw_cmds);
