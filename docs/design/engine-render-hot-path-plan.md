...
**Status:** implemented and accepted through Stage 8, the final stage
(2026-07-13); Stage 0-8 checkpoint records are generated and validated, and the
final benchmark record is in section 9\
**Owner:** Josh\
...
### Stage 8 completion record (2026-07-13) - final

```text
Stage: 8 - final acceptance and cleanup; live-projection terminology, pinned world DrawCmd budget, docs and benchmark record
Status: PASS
Checkpoint: docs/design/checkpoints/engine-render-hot-path-stage-8.json
Generated by: deno task engine:accept-stage8
Evidence: docs/design/checkpoints/evidence/engine-render-hot-path-stage-8-{debug,perf,profile}.json plus engine-render-hot-path-stage-8-final-benchmark-{perf,profile}.json
The generated checkpoint is the value-level record; this summary intentionally duplicates no hashes, samples, environment strings, or command counts.
```

Stage 8 reporting is generated, not transcribed. `engine:accept-stage8` owns the
debug build and correctness lane, release performance build, same-run profile,
golden manifest (Stage 8 authorizes no golden changes), checkpoint generation,
and validation, plus the Stage 8 extras from sections 4 and 5: an explicit
`deno task engine:build` with `wasm-opt`/`brotli` availability recorded from the
release build metadata, a full `deno task test` Playwright run against the
release-generated artifact, a `deno lint` run whose pre-existing findings are
documented as stage-owned data (all in the pre-Stage-0 engine runtime or
wasm-bindgen-generated glue, none in any file touched by Stages 0-8), the
obsolete-helper remnant check
`rg "render_legacy|smooth_player_world_pos|apply_streaming_chunks|streaming_fingerprint" game_engine`
exiting 1 with no matches, and a targeted run of the new pinned final-acceptance
test `world_draw_cmd_count_is_two_for_floor_player_mvp` (the floor/player MVP
emits exactly 2 world DrawCmds - one merged floor-atlas batch plus one player
sprite batch - within the section 7 budget of 4). Beyond the Stage 7 gates it
enforces and records: the snapshot matrix remaining all `0` across the same
eleven recorded frame paths with per-path state proof, Rust workspace test
counts with a 148-passing floor (Stage 7's 147 plus the pinned DrawCmd test) and
zero failures, the exact Stage 3 world-layer parity hash
`fnv1a64:c55ac880b00ac4d0` and Stage 4 all-resident world-state hash
`fnv1a64:718bb0099657e9aa` held exactly through the terminology cleanup (the
shadow-projection naming from the Stage 3 scaffolding era was renamed to live
projection naming; renames must be behavior-neutral), the 100 ms final
acceptance-matrix ceiling with `droppedSimTimeMs == 0` and all six drop counters
zero, a 20-frame median no worse than the Stage 7 evidence median beyond the
Stage 7 evidence's own recorded sample spread, the Stage 7 emission-cache
browser gate unchanged, and an explicit no-golden-change manifest. Stage 8 owns
the complete section 7 final acceptance matrix as recorded stage-owned data -
one entry per row with the actual recorded values, including the re-checked
Stage 2 terrain remnant command and the production RAF row gated as `/engine`
median beating the same-run `/webgl` control with the 2 ms stretch target met -
plus the dead-code cleanup summary (nothing removed beyond naming; the section 4
obsolete helpers were already deleted by Stages 1-7, and the pre-existing orphan
entity helpers `has_entity`, `spawn_entity_auto`, and `spawn_or_replace_entity`
predate Stage 0 and are documented rather than removed) and the updated
documentation list (`docs/GAME_ENGINE_ARCHITECTURE.md`,
`docs/design/engine-webgl-parity-cutover.md`, and this plan), which now describe
chunk-major authority, local projection residency, and the zero-snapshot
renderer consistently. The Stage 8 checkpoint uses the same `content` identity
as Stages 1-7, so any later content change invalidates it without rerunning
acceptance; the Stage 7 checkpoint's content digest is intentionally stale from
this point onward.

### Final benchmark record (Stage 8, 2026-07-13)

Recorded per the Stage 8 acceptance items "record final benchmark tables" and
"`engine:perf` records final `/webgl` and `/engine` median/p95 in this
document". Environment for the final columns: Chromium 148.0.7778.215, 1920x1080
viewport, DPR 1, application-context renderer ANGLE (Intel, Vulkan 1.4.348,
Intel Iris Xe RPL-U) on both routes, zero WebGL errors, no context loss. Exact
raw samples, GL health, and served-artifact provenance are in the committed
evidence artifacts
`docs/design/checkpoints/evidence/engine-render-hot-path-stage-8-final-benchmark-perf.json`
(20-frame release harness) and
`docs/design/checkpoints/evidence/engine-render-hot-path-stage-8-final-benchmark-profile.json`
(same-run RAF profile); the Stage 8 acceptance run additionally records and
validates its own independently measured
`engine-render-hot-path-stage-8-{perf,profile}.json` evidence value-for-value.

| Measurement (median / p95)  | Baseline (Stage 0, 2026-07-12) | Final (Stage 8, 2026-07-13) |
| --------------------------- | ------------------------------ | --------------------------- |
| 20-frame play-world harness | 3324.9 ms / 3638.7 ms          | 3.7 ms / 4.0 ms             |
| Production RAF `/engine`    | 468.6 ms / 482.8 ms            | 0.2 ms / 0.3 ms             |
| Same-run `/webgl` control   | 13.5 ms / 20.8 ms              | 4.4 ms / 8.0 ms             |

The harness ceiling tightened from the diagnostic 6000 ms at Stage 0 to the 100
ms final acceptance-matrix budget (Stage 7). `/engine` beats its same-run
`/webgl` control by roughly 22x on this host and meets the <= 2 ms stretch
target; `/webgl` is a same-run control, not an absolute timing oracle, and the
two baseline-column runs predate the fixed-tick scheduler, so the baseline
control differs from the final-run control.
