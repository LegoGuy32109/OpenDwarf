...
Text-input & chat design nailed in [`design/text-input-and-chat.md`](design/text-input-and-chat.md). HUD, chat (DOM-input mirror, GM-visible buffer), game menus into the session domain, on `/engine`. The existing `webgl2/` engine stays intact as the parity reference — do **not** delete its UI incrementally. Retirement is a single cutover (below) once parity is reached.
Text is **wasm-authoritative** (input-arena §8 `Text` codepoint path; the hidden `<input>` is an event source for **mobile keyboard + paste + autocorrect**, not an editor). **Capture split:** insertion/paste ← hidden `<input>` (`beforeinput`); caret/deletion ← keymap (TextField context). **IME deferred** (ASCII font prunes CJK at ingestion; `Composition` event kind reserved). One reusable text-field widget: caret + editing, **no selection** (reserved); caret/blink/scroll in the imgui-core §6 retained slot, thin I-beam caret, horizontal scroll-to-caret with **scissor clip** (first scissor consumer). Escape **cancels an active field before** reaching the shell — safe because `text_capture_active` is local-only (never network-set), preserving the Phase-3 secure-attention guarantee. Chat **draft lives in a minimal session model** (`chat_draft`, GM-visible seam projected into `ClientView` in Phase 5); bubbles + slash-command dispatch deferred to Phase 5 / session-model integration. HUD/menu porting is mechanical over the Phase-1 widgets + this field.
