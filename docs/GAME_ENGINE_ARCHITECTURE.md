...
*Design interview (pending, before implementation):* **Domains, intents & the sovereign router** — the two structurally-isolated domain stores; the `Intent` enum(s) (sovereign vs session) and how intents-out are collected + applied (sovereign local-immediate vs session/network); reserved-key routing (Escape intercepted **pre-keymap** as the secure-attention key, per [`design/input-arena.md`](design/input-arena.md) §4); and modal/focus capture rules. → new `docs/design/*.md`.
